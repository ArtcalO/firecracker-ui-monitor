...
export default{
	...
	methods:{
		fetchShopStock(){
			this.isLoading=true
			axios.get('shop-stocks/?quantity__gte=0')
			.then((res)=>{
		...
		createloss(){
			if(this.loss.quantity==0){
				this.useNotifyError("Veuillez spécifier la qunatité perdu !")
				return
			}
			this.isLoading=true

			axios.post("loss-shop/", this.loss)
			.then((res)=>{
