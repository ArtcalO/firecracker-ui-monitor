...
export default{
	...
	methods:{
		...
		createRootFile(){
			if(this.iso_file.os_id==null){
				this.useNotifyError("Veuillez selectionner OS ")
				return
			}
			if(this.iso_file.file==null){
				this.useNotifyError("Veuillez selectionner le fichier root ")
				return
			}
			let data = new FormData()
			data.append("os_id",this.iso_file.os_id)
			data.append("file",this.iso_file.file)
			axios.post("iso_files/", data)
			.then((res)=>{
				this.useNotifySuccess("iso_file Crée avec success !")
				this.$store.state.iso_files.push(res.data)
				this.$router.go(-1)
			})
			.catch((err)=>{
...
<template>
	<el-card>
		<el-form label-position="top" label-width="100px" :model="iso_file">
			<h5 class="border-b pb-3 mb-3">Création d'un fichier iso</h5>
			<el-row :gutter="20">
				<el-col :span="24" :sm="12">
                    <el-form-item label="Type d' OS">
						<el-select filterable v-model="iso_file.os_id" placeholder="Select OS" class="w-100">
							<el-option
